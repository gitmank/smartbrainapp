import { Component } from "react";
import '../components/Home.css'
import SearchBar from '../components/SearchBar';
import ImagePane from '../components/ImagePane';
import ColorChart from '../components/ColorChart';

class Home extends Component {
    constructor() {
        super()
        this.state = {
            colors: [],
            invalidLink: false,
            imageLink: 'data:image/png;base64,',
        }
    }

    onSearchChange = (event) => {
        this.setState({ invalidLink: false, imageLink: event.target.value })
    }

    onSearchSubmit = async () => {
        this.setState({ colors: [] })
        let { imageLink } = this.state;
        let receivedColors = false;
        if(imageLink.length > 5) 
        receivedColors = await this.props.getColors(imageLink);
        if(receivedColors) this.setState({ colors: receivedColors });
        else this.setState({ invalidLink: true })
    }
    render() {
        if(this.props.page==='home') {
            return(
                <>
                    <h3 className="count-display">You have { this.props.count } free tries left!</h3>
                    <SearchBar 
                        onSearchChange={this.onSearchChange} 
                        onSearchSubmit={this.onSearchSubmit}
                        invalidLink={this.state.invalidLink} 
                    />
                    <ImagePane imageLink={this.state.imageLink} />
                </>
            )
        }
        else if(this.props.page==='results') {
            return(
                <>
                    <ColorChart colors={this.state.colors} />
                </>
            )
        }
    }
}

export default Home;